...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyDown(KEY_UP)) playerPosition.z -= moveDistance;
        if (IsKeyDown(KEY_DOWN)) playerPosition.z += moveDistance;
        if (IsKeyDown(KEY_LEFT)) playerPosition.x -= moveDistance;
        if (IsKeyDown(KEY_RIGHT)) playerPosition.x += moveDistance;
